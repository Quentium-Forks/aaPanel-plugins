...
class clear_main:
    ...
    def ReturnLoginLog(self):
        ...
        clearPath = [{'path': '/usr/local/lsws/logs', 'find': ['error.log','stderr.log','lsrestart.log','access.log']}]
        for c in clearPath:
            ...
            for d in os.listdir(c['path']):
                ...
                if os.path.isfile(filename):
                    ...
                    ret_size['name'] = filename
                    ret_size['filename'] = d
                    ret_size['count_size'] = os.path.getsize(filename)
                    ret_size['size'] = self.ToSize(os.path.getsize(filename))
                    ret.append(ret_size)
        return ret
    ...
    # PHP慢日志
    def lsws_log(self):
        clearPath = [{'path': '/usr/local/lsws/logs', 'find': 'error.log'}]
        total = count = 0
        ret = []
        for c in clearPath:
            if os.path.exists(c['path']):
